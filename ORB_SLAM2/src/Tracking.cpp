...
cv::Mat Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp)
{
    ...
    // 步骤3：跟踪
        // 1:单目初始化；
        // 2:通过上一帧获得初始位姿估计或者重定位。也就是求出当前帧在世界坐标系下的位姿T 2 T_2T 
        // 3:跟踪局部地图（TrackLocalMap()） 求得位姿估计T 3 T_3T.这个步骤回答当前帧的特征点和map中的哪些mappoint匹配。
